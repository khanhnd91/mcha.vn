  ...
  border-left: 8px solid #93C7E2;">'.$cat_list['cat'].'</h3>';
			...
			foreach($cat_list['post_list'] as $post){
				if($toDay >= $post['post_date'] ){
					...
					$author_info = get_userdata($post_info->post_author);
					$author_ID   = $author_info->ID;
					$post_author = $author_info->data->display_name;
		...
		if(jpnCheck()){
			$html .= '<span class="archive-byline">Written by <a href="'.get_author_posts_url($author_ID).'" title="'.$post_author.' による投稿" rel="author">'.$post_author.'</a> on '.$post_date.'</span>';
		}else{
			$html .= '<span class="archive-byline">Translated by <a href="'.get_author_posts_url($author_ID).'" title="'.$post_author.' による投稿" rel="author">'.$post_author.'</a> on '.$post_date.'</span>';
		}
					$html .= '<p>'.$post_desc.'</p>';
					$html .= '</div>';
